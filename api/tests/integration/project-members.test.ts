...
describe('Project Members Integration', () => {
    ...
    afterAll(async () => {
        try {
            ...
            if (invited) {
                await prisma.projectMember.deleteMany({ where: { userId: invited.id } });
    ...
    it('should invite a new user and trigger email', async () => {
        ...
        const callArgs = (emailService.sendInvitationEmail as jest.Mock).mock.calls[0];
    ...
    it('should list members returning an array', async () => {
        ...
        const invitedMember = res.body.find((m: any) => m.user.email === inviteEmail);
