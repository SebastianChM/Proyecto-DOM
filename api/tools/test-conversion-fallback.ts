...
async function testConversionFallback() {
  ...
  try {
    ...
    const response = await axios.post(`${API_URL}/api/conversion/${FILE_ID}`, {
      format: "pdf",
    });
    ...
    if (
      ...
      response.data.message.includes("Model Derivative")
    ) {
      console.log("✅ SUCCESS: Fallback to Model Derivative triggered!");
    } else if (
      response.data.message &&
      response.data.message.includes("Design Automation")
    ) {
      console.log(
        "❓ Unexpected: Design Automation started? (Maybe it worked?)",
      );
    }
  } catch (error: any) {
