...
async function testPermissions(users: any, project: any) {
  ...
  const ownerPerms = await authorizationService.getUserPermissions(
    users.user1.id,
    project.id,
  );
  ...
  const noAccessPerms = await authorizationService.getUserPermissions(
    users.user2.id,
    project.id,
  );
  ...
  const adminPerms = await authorizationService.getUserPermissions(
    users.admin.id,
    project.id,
  );
...
async function testShareProject(users: any, project: any) {
  ...
  const editorPerms = await authorizationService.getUserPermissions(
    users.user2.id,
    project.id,
  );
  log(
    "✓",
    `User2 ahora tiene ${editorPerms.length} permisos como EDITOR`,
    colors.green,
  );
  ...
  const viewerPerms = await authorizationService.getUserPermissions(
    users.user3.id,
    project.id,
  );
  log(
    "✓",
    `User3 ahora tiene ${viewerPerms.length} permisos como VIEWER`,
    colors.green,
  );
...
async function testRoleChange(users: any, project: any) {
  ...
  if (member) {
    ...
    const newPerms = await authorizationService.getUserPermissions(
      users.user2.id,
      project.id,
    );
...
async function testRevokeAccess(users: any, project: any) {
  ...
  const hasAccess = await authorizationService.canAccessProject(
    users.user3.id,
    project.id,
  );
  if (!hasAccess) {
...
async function testGetMembers(users: any, project: any) {
  ...
  members.members.forEach((m: any) => {
    console.log(
      `   • ${m.name} (${m.role}) - Invitado: ${m.invitedAt ? "✓" : "✗"}`,
    );
  });
}
