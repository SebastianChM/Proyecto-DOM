...
async function cleanup() {
  ...
  await prisma.user.deleteMany({
    where: {
      email: {
        in: [
          "test-user-a@test.com",
          "test-user-b@test.com",
          "test-admin@test.com",
        ],
      },
    },
...
async function runTests() {
  try {
    ...
    log(
      "🔗",
      "FASE 4: Usuario A comparte Project A2 con Usuario B (como VIEWER)...",
    );
    ...
    if (
      projectsUserA.length === 2 &&
      JSON.stringify(userAProjectNames) ===
        JSON.stringify(expectedUserAProjects)
    ) {
      pass(
        "Usuario A ve sus 2 proyectos",
        expectedUserAProjects,
        userAProjectNames,
      );
    } else {
      fail(
    ...
    log(
      "🧪",
      "TEST 2: Usuario B debe ver sus 2 proyectos + 1 compartido (3 total)...",
    );
    ...
    if (
      projectsUserB.length === 3 &&
      JSON.stringify(userBProjectNames) ===
        JSON.stringify(expectedUserBProjects)
    ) {
      pass(
        "Usuario B ve sus 2 proyectos + 1 compartido",
        expectedUserBProjects,
        userBProjectNames,
      );
    } else {
      fail(
    ...
    if (!userASeesB1 && !userASeesB2) {
      ...
      fail(
        "Usuario A NO ve proyectos de B",
        false,
        true,
    ...
    if (!userBSeesA1) {
      ...
      fail(
        "Usuario B NO ve proyecto privado A1",
        false,
        true,
    ...
    if (userBSeesA2) {
      ...
      fail(
        "Usuario B SÍ ve proyecto compartido A2",
        true,
        false,
    ...
    if (projectsAdmin.length === 4) {
      log(
        "ℹ️",
        "INFO: Admin puede ver todos los proyectos mediante query directa",
      );
      pass("Admin puede ver todos (query directa)", 4, projectsAdmin.length);
    } else {
      log(
        "ℹ️",
        "INFO: Query de admin no implementada para ver todos los proyectos",
      );
    }
    ...
    if (
      ...
      pass(
        "Roles de ProjectMember correctos",
        { a1: "OWNER", a2Owner: "OWNER", a2Viewer: "VIEWER" },
        {
          a1: memberA1.role,
          a2Owner: memberA2Owner.role,
          a2Viewer: memberA2Viewer.role,
        },
      );
    } else {
      fail(
