...
function addFillerPages(doc, count) {
    for (let i = 0; i < count; i++) {
        ...
        doc.text("GENERAL CONDITIONS AND REQUIREMENTS - BOILERPLATE CONTENT", { align: 'center' });
        doc.moveDown();
        // Add dense text
        for (let j = 0; j < 4; j++) {
...
// --- Title Page ---
doc.addPage();
doc.font('Helvetica-Bold').fontSize(24).text('PROJECT: RESIDENTIAL TOWER - PHASE 1', { align: 'center' });
doc.moveDown();
doc.fontSize(18).text('DOCUMENT: TECHNICAL SPECIFICATIONS', { align: 'center' });
...
doc.addPage();
doc.font('Helvetica-Bold').fontSize(16).text('SECTION 03.30 - CAST-IN-PLACE CONCRETE');
doc.moveDown();
...
doc.addPage();
doc.font('Helvetica-Bold').fontSize(16).text('SECTION 08.11 - METAL DOORS AND FRAMES');
doc.moveDown();
doc.font('Helvetica').fontSize(12);
