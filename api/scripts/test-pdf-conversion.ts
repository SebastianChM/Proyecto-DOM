...
async function runTest() {
    ...
    const urn = 'dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6aWRvbS1iaW0tcGxhdGZvcm0tdXMtdGVzdC0wMDEvMTc2NTQ2NDY3NjEyOC1TQ0ZBLVBCLUNDQS1BUlEtR0VOLVBMUC0xMDAxLmR3Zw';
    ...
    try {
        ...
        console.log('✅ Job submitted successfully:', JSON.stringify(jobResult, null, 2));
        ...
        for (let i = 1; i <= maxAttempts; i++) {
            ...
            try {
                const manifest = await modelDerivativeService.getManifest(urn);
                console.log(`[Attempt ${i}/${maxAttempts}] Status: ${manifest.status}, Progress: ${manifest.progress}`);

                if (manifest.status === 'success') {
                    console.log('🎉 CONVERSION SUCCESS!');

                    // Check if derivatives contain PDF
                    const hasPdf = manifest.derivatives?.some((d: any) => d.outputType === 'pdf');
                    if (hasPdf) {
                        console.log('✅ PDF output confirmed in manifest.');
                    } else {
                        console.warn('⚠️ Manifest status is success but no outputType="pdf" found directly.');
                    }
                    return;
                }
