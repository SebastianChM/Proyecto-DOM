...
async function verifyUniversalParser() {
    ...
    console.log(`✅ Processed ${result.totalLinesProcessed} lines in ${result.parseDurationMs}ms`);
    console.log(`✅ Found ${result.requirements.length} Requirements`);

    // 3. Inspect Results (Show Grammar Type)
    console.log('\n--- Extraction Report ---');
    result.requirements.forEach(req => {
        console.log(`[${req.grammarType}] ${req.parameter} ${req.operator} ${req.value}`);
        if (req.normalized) {
            console.log(`   └─ 📏 SI Unit: ${req.normalized.value} ${req.normalized.unit}`);
        }
