...
async function verifyValidation() {
    ...
    const wallPassvStrength = results.find(r => r.elementId === '1001' && r.property === 'Compressive Strength' && r.status === 'PASS');
    const wallPassThickness = results.find(r => r.elementId === '1001' && r.property === 'Thickness' && r.status === 'PASS');

    // Check Element B (Floor)
    const floorFailThickness = results.find(r => r.elementId === '1002' && r.property === 'Thickness' && r.status === 'FAIL');

    // Check Element C (Door)
    const doorPassFire = results.find(r => r.elementId === '1003' && r.property === 'Fire Rating' && r.status === 'PASS');

    // Check Confusion (Door should NOT fail on Concrete specs)
    const doorFailConcrete = results.find(r => r.elementId === '1003' && r.property === 'Thickness');

    console.log('\n--- Results ---');
    console.log(`✅ Wall Strength Match: ${wallPassvStrength ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Wall Thickness Match (Unit Conversion): ${wallPassThickness ? 'PASS' : 'FAIL'} (Expected 200mm == 0.20m)`);
    console.log(`✅ Floor Thickness Mismatch: ${floorFailThickness ? 'PASS' : 'FAIL'} (Expected Fail)`);
    console.log(`✅ Door Rating Match: ${doorPassFire ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Context Awareness: ${!doorFailConcrete ? 'PASS (Door ignored concrete specs)' : 'FAIL (Door checked against concrete)'}`);

    if (wallPassvStrength && wallPassThickness && floorFailThickness && doorPassFire && !doorFailConcrete) {
