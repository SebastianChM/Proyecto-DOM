...
async function main() {
    ...
    realFiles.forEach(f => {
        ...
        console.log(`  Viewer URL: http://localhost:3000/dashboard/viewer?urn=${f.apsUrn}`);
        console.log('');
    });

    if (realFiles.length === 0) {
        console.log('⚠️  No real BIM files found. All files appear to be mock/local files.');
        console.log('To test the 3D viewer properly, you need to upload a real file from Autodesk.');
    }
}
