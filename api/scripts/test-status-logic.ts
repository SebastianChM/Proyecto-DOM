...
async function testStatusLogic() {
    ...
    if (files.length === 0) {
        ...
        console.log('🔍 Checking most recent file to see its real status on APS...');
        const recentFile = await prisma.file.findFirst({
            where: { apsUrn: { not: null } },
            orderBy: { createdAt: 'desc' }
        });
        
        if (recentFile && recentFile.apsUrn && !recentFile.apsUrn.startsWith('local-')) {
            console.log(`📄 Checking file: ${recentFile.name} (Status: ${recentFile.status})`);
