...
function findAllPdfs(children: Child[], path: string = ''): string[] {
    ...
    for (const child of children) {
        ...
        if (child.mime?.includes('pdf') || child.role === 'pdf' || child.urn?.includes('.pdf')) {
        ...
        if (child.role === '2d') {
            ...
            if (child.children) {
                ...
                child.children.forEach((c, i) => {
                    console.log(`     ${i + 1}. Role: ${c.role}, MIME: ${c.mime}, Type: ${c.type}`);
...
async function main() {
    ...
    try {
        ...
        console.log(`Total top-level derivatives: ${manifest.derivatives?.length || 0}\n`);
        ...
        const svf2 = manifest.derivatives?.find((d: any) => d.outputType === 'svf2');
        if (svf2?.children) {
