...
async function analyzeMopSpec() {
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (sectionRegex.test(line)) {
                ...
                if (match) {
                    ...
                    if (match[2].toLowerCase().includes('hormogón') || match[2].toLowerCase().includes('hormigon')) {
        ...
        fs.writeFileSync(dumpPath, structure.map(s => `${s.code} ${s.title}`).join('\n'));
        console.log(`📄 Structure Dumped to: ${dumpPath}`);

        if (!concreteSectionFound) {
            console.warn('⚠️ Warning: No explicit "Hormigón" section title found in the scan. MOP might use "Pavimentos" or other terms.');
        }
