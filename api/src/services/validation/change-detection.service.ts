 ...
 */
export async function detectAndNotifyChanges(fileId: string, latestValidationId: string, userId: string, projectId: string | null) {
    try {
        ...
        if (latest.missingCount !== previous.missingCount) {
            ...
            changesDetected.push(`Missing elements changed: ${diff > 0 ? '+' : ''}${diff}`)
        }
        ...
        if (latest.undocumentedCount !== previous.undocumentedCount) {
            ...
            changesDetected.push(`Undocumented changed: ${diff > 0 ? '+' : ''}${diff}`)
        }
        ...
        // Find new issues
        const newIssues = latest.issues.filter(i =>
            i.elementTag && !previousIssuesMap.has(i.elementTag)
        )

        // Find resolved issues
        const resolvedIssues = previous.issues.filter(i =>
            i.elementTag && !latestIssuesMap.has(i.elementTag)
        )

        // If significant changes detected, create a FILE_CHANGED notification
        if (changesDetected.length > 0 || newIssues.length > 0 || resolvedIssues.length > 0) {
            ...
            await prisma.notification.create({
                data: {
                    ...
                    metadata: JSON.stringify({
                        newIssues: newIssues.map(i => ({ tag: i.elementTag, type: i.type })),
                        resolvedIssues: resolvedIssues.map(i => ({ tag: i.elementTag, type: i.type })),
            ...
            console.log(`✅ Change detection: ${changesDetected.length} changes, ${newIssues.length} new, ${resolvedIssues.length} resolved`)
        }
    } catch (error) {
