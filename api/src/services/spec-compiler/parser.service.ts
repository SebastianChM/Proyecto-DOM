...
import { Token, TokenType, ASTNode, ASTNodeType, UniversalRequirement, GrammarType } from '../../types/spec-grammar.types';
...
export class ParserService {
    ...
    private matchExplicit(tokens: Token[]): ASTNode | null {
        ...
        const modalIndex = tokens.findIndex(t => t.type === TokenType.KEYWORD_MODAL);
        ...
        const hasEntityOrParam = lhsTokens.some(t =>
            t.type === TokenType.KEYWORD_ENTITY || t.type === TokenType.KEYWORD_PARAM
        );
    ...
    public compileToRequirement(node: ASTNode, originalText: string, page: number): UniversalRequirement {
        // Logic to extraction parameter/value from Tokens
        const tokens = node.tokens || [];

        const op = tokens.find(t => t.type === TokenType.OPERATOR)?.value || 'MUST';
        const valToken = tokens.find(t => t.type === TokenType.NUMERIC);
        const unitToken = tokens.find(t => t.type === TokenType.UNIT);
        const paramToken = tokens.find(t => t.type === TokenType.KEYWORD_PARAM || t.type === TokenType.KEYWORD_ENTITY);
