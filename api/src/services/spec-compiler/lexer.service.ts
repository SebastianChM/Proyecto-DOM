...
import {
    getAllEquipmentTerms,
    getAllParameterTerms,
    normalizeParameter
} from '../../config/electrical-vocabulary';
...
export class LexerService {
    ...
    private readonly KEYWORDS_MODAL = [
        'SHALL', 'MUST', 'SHOULD', 'REQUIRED', 'REQUIRES', 'WILL',
        'DEBE', 'DEBERÁ', 'DEBERA', 'OBLIGATORIO', 'REQUERIDO', 'SERÁ', 'SERA'
    ];

    // Entity keywords (English + Spanish, includes electrical equipment)
    private readonly KEYWORDS_ENTITY = [
        // Structural
        'CONCRETE', 'STEEL', 'WALL', 'COLUMN', 'BEAM', 'SLAB', 'REBAR', 'CEMENT',
        'HORMIGÓN', 'HORMIGON', 'ACERO', 'MURO', 'COLUMNA', 'VIGA', 'LOSA',
        // Electrical
        'CABLE', 'CONDUCTOR', 'WIRE', 'TRAY', 'BANDEJA', 'CONDUIT', 'TUBO',
        'PANEL', 'TABLERO', 'TRANSFORMER', 'TRANSFORMADOR', 'LUMINAIRE', 'LUMINARIA',
        'OUTLET', 'TOMACORRIENTE', 'SWITCH', 'INTERRUPTOR', 'BREAKER',
        'LIGHTING', 'ILUMINACIÓN', 'ILUMINACION'
    ];

    // Parameter keywords (English + Spanish)
    private readonly KEYWORDS_PARAM = [
        // Dimensions
        'STRENGTH', 'THICKNESS', 'WIDTH', 'HEIGHT', 'DEPTH', 'LENGTH', 'DIAMETER', 'SIZE', 'GAUGE',
        'RESISTENCIA', 'ESPESOR', 'ANCHO', 'ALTO', 'ALTURA', 'PROFUNDIDAD', 'LARGO', 'LONGITUD', 'DIÁMETRO', 'DIAMETRO', 'CALIBRE',
        // Electrical
        'VOLTAGE', 'CURRENT', 'POWER', 'FREQUENCY', 'PHASE', 'AMPACITY', 'CAPACITY', 'LOAD',
        'VOLTAJE', 'TENSIÓN', 'TENSION', 'CORRIENTE', 'POTENCIA', 'FRECUENCIA', 'FASE', 'AMPERAJE', 'CAPACIDAD', 'CARGA',
        // General
        'GRADE', 'CLASS', 'SPACING', 'TYPE', 'MATERIAL',
        'GRADO', 'CLASE', 'ESPACIAMIENTO', 'TIPO'
    ];

    // Operators (English + Spanish)
    private readonly OPERATORS = [
        '>=', '<=', '>', '<', '=', '≥', '≤',
        'AT LEAST', 'AT MOST', 'MINIMUM', 'MAXIMUM', 'EXCEED', 'LESS THAN', 'MORE THAN',
        'MÍNIMO', 'MINIMO', 'MÁXIMO', 'MAXIMO', 'AL MENOS', 'NO MAYOR', 'NO MENOR', 'MAYOR QUE', 'MENOR QUE'
    ];
    ...
    private readonly UNITS = [
        ...
        'MPA', 'PSI', 'KPA',
        // Electrical
        'V', 'KV', 'MV', 'A', 'MA', 'KA', 'W', 'KW', 'MW', 'VA', 'KVA', 'MVA', 'HZ', 'KHZ',
        // Area
        'MM2', 'CM2', 'M2', 'MM²', 'CM²', 'M²', 'AWG', 'MCM',
        // Other
        'KG', 'LBS', '%', 'DEGREE', 'GRADO'
    ];
    ...
    public tokenize(input: string): Token[] {
        ...
        while (this.pos < this.input.length) {
            ...
            // 3. Punctuation
            if (/[;:.,()]/.test(char)) {
                tokens.push({ type: TokenType.PUNCTUATION, value: char, position: this.pos++ });
                continue;
            }
            ...
            if (/[≥≤²³°%]/.test(char)) {
                if (char === '≥') {
                    tokens.push({ type: TokenType.OPERATOR, value: '>=', position: this.pos++ });
                } else if (char === '≤') {
                    tokens.push({ type: TokenType.OPERATOR, value: '<=', position: this.pos++ });
                } else {
                    tokens.push({ type: TokenType.TEXT, value: char, position: this.pos++ });
                }
                continue;
            }
    ...
    private tryReadOperator(): Token | null {
        ...
        for (const op of sortedOps) {
            if (remaining.startsWith(op)) {
                ...
                const token = { type: TokenType.OPERATOR, value: op, position: this.pos };
                this.pos += op.length;
                return token;
            }
        }
    ...
    private readWord(): Token {
        ...
        while (this.pos < this.input.length && /[a-zA-Z0-9_\-áéíóúñÁÉÍÓÚÑ]/.test(this.input[this.pos])) {
            this.pos++;
        }
        ...
        if (this.KEYWORDS_PARAM.includes(upper)) {
            return { type: TokenType.KEYWORD_PARAM, value: normalizeParameter(value), position: start };
        }
