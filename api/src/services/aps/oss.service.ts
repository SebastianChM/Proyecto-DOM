...
export class ApsOssService {
        ...
        try {
          ...
          throw new Error(
            "Failed to create APS bucket: " +
              (createError.response?.body?.reason || createError.message),
          );
        }
      } else {
        throw error;
  ...
  async getSignedUrl(objectName: string) {
    ...
    try {
      ...
      throw new Error(
        `Failed to get signed URL: ${error.response?.body?.reason || error.message}`,
      );
    }
  }
  ...
  async getSignedWriteUrl(objectName: string) {
    ...
    try {
      ...
      throw new Error(
        `Failed to get signed write URL: ${error.response?.body?.reason || error.message}`,
      );
    }
  }
