...
import { UniversalRequirement, GrammarType, ExtractionResult } from '../types/spec-grammar.types';
...
export class SpecParserService {
    ...
    parse(text: string): ExtractionResult {
        ...
        const narrativeRegex = /([A-Z][a-zA-Z0-9\s]+?)\s+(shall be|must be|is required to be|debe ser|será|deberá ser)\s+(.+?)(?:[\.;]|$)/i;

        // B. Key-Value Strategy: "Parameter: Value" or "Parameter = Value"
        const keyValueRegex = /^([•\-]?\s*[A-Z][a-zA-Z0-9\s\(\)\.]{2,50})\s*(?:=|:)\s*([0-9\.,]+\s*[A-Za-z%\/]+)/i;

        // C. Numeric Limit Strategy: "Max/Min Parameter Value"
        // Captures: "Max Water Ratio 0.5", "Espesor Mínimo 20cm"
        const limitRegex = /(Max|Min|Máximo|Mínimo)\s+([A-Za-z0-9\s]+)\s+([0-9\.,]+)/i;
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (narrativeMatch) {
                requirements.push(this.createReq(i, line, GrammarType.NARRATIVE, narrativeMatch[1], narrativeMatch[2], narrativeMatch[3], currentCategory));
                continue;
            }
            ...
            if (kvMatch) {
                requirements.push(this.createReq(i, line, GrammarType.KEY_VALUE_PAIR, kvMatch[1], 'EQUALS', kvMatch[2], currentCategory));
                continue;
            }
            ...
            if (limitMatch) {
                requirements.push(this.createReq(i, line, GrammarType.KEY_VALUE_PAIR, `${limitMatch[1]} ${limitMatch[2]}`, 'LIMIT', limitMatch[3], currentCategory));
                continue;
            }
        }
    ...
    private createReq(lineIdx: number, text: string, type: GrammarType, param: string, op: string, val: string, cat: string): UniversalRequirement {
        ...
        return {
            ...
            value: cleanValue,
            normalized: norm ? { value: norm.standardValue, unit: norm.standardUnit } : undefined,
            derivedCategory: cat,
