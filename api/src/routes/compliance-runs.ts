...
import { complianceRunnerService, BimElement } from '../services/compliance-runner.service';
...
router.post('/', async (req: Request, res: Response) => {
    try {
        ...
        console.log(`[ComplianceAPI] Starting run: ruleset=${rulesetId}, elements=${elements.length}`);
    ...
    try {
        ...
        if (!bimProperties || bimProperties.length === 0) {
            return res.status(400).json({
                error: 'No properties could be extracted from the model. Make sure the model is processed.'
            });
        }

        console.log(`[ComplianceAPI] Extracted ${bimProperties.length} elements from model`);
        ...
        console.log(`[ComplianceAPI] Model compliance complete. Score: ${result.complianceScore}%, Issues: ${result.issues?.length || 0}`);
        ...
        if (error.message?.includes('No Property Database')) {
            return res.status(400).json({
                error: 'Model properties not yet available. Please wait for model processing to complete.'
            });
        }
...
router.get('/:id/issues', async (req: Request, res: Response) => {
    try {
        ...
        const whereClause: any = { runId: id };
        if (severity) whereClause.severity = severity;
        if (category) whereClause.elementCategory = { contains: category as string };
 ...
 */
function generateDemoElements(ruleset: any): BimElement[] {
            ...
            for (const rule of ruleset.rules) {
                if (rule.targetCategory === category) {
                    ...
                    if (!isNaN(expectedNum)) {
                        ...
                        if (Math.random() > 0.4) {
                            // Passing value
                            properties[rule.propertyName] = expectedNum + (Math.random() - 0.5) * 10;
                        } else {
                            // Failing value (significantly different)
                            properties[rule.propertyName] = expectedNum * (0.3 + Math.random() * 0.4);
                        }
                    } else {
                        properties[rule.propertyName] = Math.random() > 0.4 ? rule.expectedValue : 'Invalid';
                    }
                }
            }
