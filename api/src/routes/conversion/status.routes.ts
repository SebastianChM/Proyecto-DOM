        ...
        if (manifest.status === "success") {
          ...
          if (conversion.targetFormat === "pdf") {
            ...
            if (derivative?.children) {
              foundResource = derivative.children.find(
                (c: any) => c.mime === "application/pdf" || c.role === "pdf",
              );
            }
            ...
            if (!foundResource) {
              console.log(
                ...
                JSON.stringify(
                  manifest.derivatives?.map((d: any) => ({
                    outputType: d.outputType,
                    status: d.status,
                    childrenTypes: d.children?.map((c: any) => ({
                      role: c.role,
                      mime: c.mime,
                    })),
                  })),
            ...
            const derivative = manifest.derivatives?.find(
              (d: any) => d.outputType === conversion.targetFormat,
            );
            if (derivative?.children) {
              foundResource = derivative.children.find(
                (c: any) =>
  ...
  try {
    ...
    if (conversion.resultUrn?.startsWith("oss:")) {
      ...
      try {
        ...
        console.error(
          "OSS download error:",
          ossError.response?.data || ossError.message,
        );
        return res
          .status(500)
