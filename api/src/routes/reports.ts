...
router.get('/validation/:runId', async (req, res, next) => {
    try {
        ...
        const reportData = {
            ...
            missingCount: run.issues.filter((i: ValidationIssue) => i.type === 'MISSING_IN_MODEL').length,
            mismatchCount: run.issues.filter((i: ValidationIssue) => i.type === 'PROPERTY_MISMATCH').length,
            undocumentedCount: run.issues.filter((i: ValidationIssue) => i.type === 'UNDOCUMENTED_IN_TABLE').length,
        ...
        res.setHeader('Content-Disposition', `attachment; filename=validation-report-${runId.substring(0, 8)}.pdf`);
        res.send(pdfBuffer);
