...
async function verifyConnection() {
    ...
    try {
        const auth = new AuthClientTwoLegged(clientId, clientSecret, ['bucket:read', 'data:read']);
        console.log('🔄 Requesting 2-Legged Token (Client Credentials)...');

        const credentials = await auth.authenticate();

        console.log('✅ Connection Successful!');
        console.log('🎫 Access Token acquired:', credentials.access_token.substring(0, 10) + '...');
