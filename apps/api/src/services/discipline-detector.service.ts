...
export class DisciplineDetectorService {
  ...
  detectFromFilename(filename: string): DisciplineResult {
    ...
    for (const [discipline, patterns] of Object.entries(
      this.FILENAME_PATTERNS,
    ) as [Discipline, RegExp[]][]) {
      ...
      for (const pattern of patterns) {
        if (pattern.test(filename)) {
          score++;
          indicators.push(
            `Filename matches ${discipline} pattern: ${pattern.source}`,
          );
        }
      }
  ...
  detectFromContent(text: string): DisciplineResult {
    ...
    const confidence = Math.min(
      0.4 + scoreDiff * 0.05 + bestScore * 0.02,
      0.95,
    );
  ...
  detect(filename: string, content: string): DisciplineResult {
    ...
    if (
      fromFilename.discipline === fromContent.discipline &&
      fromFilename.discipline !== "GENERAL"
    ) {
      return {
        discipline: fromFilename.discipline,
        confidence: Math.min(
          fromFilename.confidence + fromContent.confidence,
          0.98,
        ),
