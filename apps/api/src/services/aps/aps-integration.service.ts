...
import { apsCacheService, ApsCacheKeys, ApsCacheOptions } from "./aps-cache";
...
export class ApsIntegrationService {
  ...
  async getHubsForUser(
    ...
    // 1. Get user context (before any service calls)
    ...
    return await apsCacheService.getOrFetch(
      cacheKey,
      async () => {
        logger.debug("[APS_INTEGRATION] Fetching hubs from APS", {
  ...
  async getProjectsForHub(
    ...
    return await apsCacheService.getOrFetch(
      cacheKey,
      async () => {
        logger.debug("[APS_INTEGRATION] Fetching projects from APS", {
          userId: userId.substring(0, 8),
  ...
  async getFolderContents(
    ...
    return await apsCacheService.getOrFetch(
      cacheKey,
      async () => {
        logger.debug("[APS_INTEGRATION] Fetching folder contents from APS", {
          userId: userId.substring(0, 8),
  ...
  async invalidateUserCache(userId: string): Promise<void> {
    ...
    logger.debug("[APS_INTEGRATION] Cache invalidated for user", {
      userId: userId.substring(0, 8),
    });
  }
}
