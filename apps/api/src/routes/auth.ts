...
router.get("/callback", async (req, res) => {
  try {
    ...
    if (req.session) {
      ...
      if (env.NODE_ENV !== "production") {
        ...
        console.log("[DEBUG] Auth Callback:", {
          ...
          isOverLimit: sessionSize > 4000,
        });
        if (sessionSize > 4000) {
          console.error(
            "🚨 CRITICAL: Session size exceeds 4KB cookie limit! This will cause an auth loop.",
          );
        }
      }
    }
    ...
    console.log("Saving Auth Session before redirect...");
