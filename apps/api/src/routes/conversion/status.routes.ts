...
router.get("/:conversionId", async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      conversion.resultUrn?.startsWith("da-workitem:")
    ) {
      const [, workItemId, outputObjectKey] = conversion.resultUrn.split(":");

      try {
        const statusResponse =
          await designAutomationService.getWorkItemStatusRest(workItemId);
        const status = statusResponse as { status?: string };
      ...
      try {
        ...
        if (manifest.status === "success") {
          ...
          if (conversion.targetFormat === "pdf") {
            ...
            let derivative = manifest.derivatives?.find(
              (d: { outputType?: string }) => d.outputType === "pdf",
            );

            // If not found, check SVF2 derivatives (when using 2dviews: pdf option)
            if (!derivative) {
              derivative = manifest.derivatives?.find(
                (d: { outputType?: string; children?: unknown[] }) =>
                  d.outputType === "svf2" &&
                  d.children?.some((c: unknown) => {
                    const child = c as { mime?: string; role?: string };
                    return (
                      child.mime === "application/pdf" || child.role === "pdf"
                    );
                  }),
              );
            }
            ...
            if (derivative?.children) {
              foundResource = (derivative.children as unknown[]).find(
                (c: unknown) => {
                  const child = c as { mime?: string; role?: string };
                  return (
                    child.mime === "application/pdf" || child.role === "pdf"
                ...
                JSON.stringify(
                  manifest.derivatives?.map(
                    (d: {
                      ...
                      status: d.status,
                      childrenTypes: d.children?.map((c: unknown) => {
                        const child = c as { role?: string; mime?: string };
                        return {
                          role: child.role,
                          mime: child.mime,
                        };
            ...
            if (derivative?.status === "success" && derivative?.children) {
              foundResource = (derivative.children as unknown[]).find(
                (c: unknown) => (c as { role?: string }).role === "ifc",
              );
            }
          } else {
            const derivative = manifest.derivatives?.find(
              ...
              foundResource = (derivative.children as unknown[]).find(
                (c: unknown) => {
                  ...
                  return (
                    ...
                    child.role === conversion.targetFormat
                  );
                },
              );
            }
          ...
          if (foundResource) {
            const resource = foundResource as { urn?: string };
            await prisma.conversion.update({
              where: { id: conversionId },
              data: {
                status: "COMPLETED",
                completedAt: new Date(),
                resultUrn: resource.urn,
                resultUrl: `/api/conversion/${conversionId}/download`,
              },
            });
            conversion.status = "COMPLETED";
...
router.get("/:conversionId/download", async (req: Request, res: Response) => {
  try {
    ...
    // Handle Local Mode
    if (
      conversion.file.apsUrn?.startsWith("local-") ||
      conversion.resultUrn?.startsWith("local-mock-")
    ) {
    ...
    if (conversion.resultUrn?.startsWith("oss:")) {
      ...
      try {
        ...
        return res
          .status(500)
          .json({ error: "Failed to download PDF from storage" });
      }
    }
    ...
    try {
      const fileBuffer = await modelDerivativeService.getDerivative(
        conversion.file.apsUrn!,
      ...
      const err = downloadError as { message?: string };
      console.error("Download error:", err.message || "Unknown error");
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const baseFilename = conversion.file.name.replace(/\.[^/.]+$/, "");
      const newFilename = `${baseFilename}.${conversion.targetFormat}`;
      ...
      if (conversion.resultUrn?.startsWith("oss:")) {
        console.log(
          `⚡ FAST PATH: Using server-side copy for Design Automation result`,
        );
        const [, bucketAndKey] = conversion.resultUrn.split("oss:");
        ...
        if (bucket === BUCKET_KEY) {
          try {
            ...
            uploadedObject = await apsOssService.copyObject(
              sourceObjectKey,
              newObjectKey,
            );
            fileSize = uploadedObject?.size ?? 0;
            console.log(`✅ Server-side copy complete. Size: ${fileSize}`);
          } catch (copyError) {
            console.warn(
              "⚠️ Server-side copy failed, falling back to download/upload:",
              copyError,
            );
          }
        }
      ...
      if (!uploadedObject) {
        ...
        if (
          ...
          if (fs.existsSync(localPath)) {
            ...
            uploadedObject = await apsOssService.uploadBuffer(
              fileBuffer,
              newFilename,
            );
          } else {
        ...
        if (!uploadedObject) {
          return res
            .status(400)
            .json({ error: "Could not retrieve or upload file content" });
        }
      }

      // Create File record in Prisma
      const urn = uploadedObject.objectId
        ? apsOssService.getDerivativeUrn(uploadedObject.objectId)
        : `local-${Date.now()}`;
