...
router.post("/batch-download", async (req, res) => {
  try {
    ...
    for (const file of files) {
      try {
        ...
        if (file.apsUrn.startsWith("local-")) {
          ...
          archive.append(
            `This is a placeholder for local file: ${file.name}\nOriginal size: ${file.size} bytes.\n`,
            { name: `${file.name}.txt` },
          );
          continue;
        }
        ...
        if (!signedUrl) {
          ...
          archive.append(
            `File ${file.name} skipped: Failed to retrieve download URL.\n`,
            { name: `${file.name}.txt` },
          );
          continue;
        }
        ...
        console.error(
          `Error processing file ${file.name} for zip:`,
          errorMessage,
        );
        archive.append(`Error downloading file: ${errorMessage}\n`, {
    ...
    if (!res.headersSent) {
      res
        .status(500)
        .json({ error: "Batch download failed", details: errorMessage });
    }
  }
});
  ...
  try {
    ...
    try {
      ...
      if (file.s3Key === "IMPORTED_FROM_APS") {
        ...
        const userToken = (req as RequestWithSession).session?.user
          ?.apsAccessToken;
        ...
        if (file.apsProjectId && file.apsItemId && userToken) {
          try {
            ...
            console.warn(
              "Failed to get download URL via Data Management API:",
              err,
            );
          }
        }
        ...
        if (!signedUrl && file.apsStorageId) {
          ...
          if (storageId.startsWith("https://")) {
            ...
            storageId.startsWith("urn:adsk.objects:") ||
            storageId.startsWith("urn:adsk.wipprod:")
          ) {
            // Try to extract object key and get signed URL
            const storageMatch = storageId.match(
              /urn:adsk\.objects:os\.object:[^/]+\/(.+)/,
            );
            if (storageMatch) {
              try {
        ...
        if (!signedUrl) {
          ...
          return res.status(400).json({
            ...
            details:
              "Unable to retrieve download URL. The file may no longer be accessible in ACC/BIM360.",
          });
        }
      } else {
        ...
        const decodedUrn = Buffer.from(file.apsUrn, "base64").toString("utf-8");
        console.log(
          `[Download Debug] File ID: ${file.id}, Decoded URN: ${decodedUrn}`,
        );
        ...
        if (match) {
          objectKey = match[1];
          console.log(
            `[Download Debug] Extracted objectKey from URN: ${objectKey}`,
          );
        } else {
          ...
          if (file.s3Key && file.s3Key !== "unknown_key") {
            ...
            console.log(
              `[Download Debug] Using s3Key filename as objectKey: ${objectKey}`,
            );
          } else {
            console.error(
              `[Download Debug] Cannot determine objectKey. URN: ${decodedUrn}, s3Key: ${file.s3Key}`,
            );
            return res.status(400).json({
      ...
      if (
        ...
        return res.status(404).json({
          ...
          details:
            "El archivo ha expirado en el almacenamiento de Autodesk (política transient de 24 horas). Por favor, vuelve a subir el archivo.",
          code: "FILE_EXPIRED",
        });
      }
...
router.post("/import-aps", async (req, res) => {
  try {
    ...
    if (apsItemId) {
      ...
      if (existingFile) {
        console.log(
          `File ${name} already imported (ID: ${existingFile.id}). Returning existing record.`,
        );
        return res.json({
          success: true,
          file: existingFile,
    ...
    const dbFile = await prisma.file.create({
      ...
      } as Parameters<typeof prisma.file.create>[0]["data"], // Use Prisma's generated type
    });
...
router.get("/:id/bom", async (req, res) => {
  try {
    ...
    // Helper to safely get a nested property group as a Record
    ...
    const viewGeometry =
      metadata.data.metadata.find(
        (m: MetadataView) => m.role === "3d" && m.isMasterView,
      ) ||
      metadata.data.metadata.find((m: MetadataView) => m.role === "3d") ||
    ...
    // 2. Get Properties
    const properties = await modelDerivativeService.getProperties(
      file.apsUrn,
      guid,
    );
    ...
    if (!properties || !properties.data || !properties.data.collection) {
      console.warn(
        "BOM Extraction: No properties collection found for URN:",
        file.apsUrn,
      );
      return res.json([]);
    }
    ...
    console.log(
      `📦 BOM: Found ${properties.data.collection.length} elements in properties`,
    );
    ...
    const bom = properties.data.collection
      .map((item: PropertyItem) => {
        ...
        const category =
          ...
          (getPropertyGroup(props, "Identity Data") &&
            findProperty(
              getPropertyGroup(props, "Identity Data")!,
              "Category",
            )) ||
          (getPropertyGroup(props, "Datos de identidad") &&
            findProperty(
              getPropertyGroup(props, "Datos de identidad")!,
              "Categoría",
            )) ||
        ...
        const material =
          ...
          (getPropertyGroup(props, "Materials") &&
            findProperty(
              getPropertyGroup(props, "Materials")!,
              "Material",
              "Name",
            )) ||
          (getPropertyGroup(props, "Materiales") &&
            findProperty(
              getPropertyGroup(props, "Materiales")!,
              "Material",
              "Nombre",
        ...
        const volume = parseNumeric(
          findProperty(
            ...
            (getPropertyGroup(props, "Dimensions") &&
              findProperty(
                getPropertyGroup(props, "Dimensions")!,
                "Volume",
                "Gross Volume",
              )) ||
            (getPropertyGroup(props, "Dimensiones") &&
              findProperty(
                getPropertyGroup(props, "Dimensiones")!,
                "Volumen",
              )) ||
        ...
        const area = parseNumeric(
          findProperty(
            ...
            (getPropertyGroup(props, "Dimensiones") &&
              findProperty(getPropertyGroup(props, "Dimensiones")!, "Área")) ||
            0,
        );

        const length = parseNumeric(
          findProperty(props, "Length", "Longitud", "Curve Length") ||
            (getPropertyGroup(props, "Dimensions") &&
              findProperty(getPropertyGroup(props, "Dimensions")!, "Length")) ||
            (getPropertyGroup(props, "Dimensiones") &&
              findProperty(
                getPropertyGroup(props, "Dimensiones")!,
                "Longitud",
      ...
      .filter(
        (item: { name: string }) =>
          item.name && !item.name.startsWith("Non-Revit"),
      );
