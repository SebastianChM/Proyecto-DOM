...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `ET document uploaded: ${etDocument.id} - ${etDocument.name}`,
      );
...
router.post("/validate", async (req: Request, res: Response) => {
  try {
    ...
    if (modelFiles.length === 0) {
      console.log(
        "No models with URNs found. Generating demo results from document specs...",
      );
      ...
      if (specs.length > 0) {
        ...
        allResults = [
          ...
          },
          {
            ...
            expectedValue: "120 mins",
            actualValue: "Not specified",
            status: "WARNING",
            elementId: "1003",
            elementName: "Wall-003",
            modelName: "Demo",
            modelId: "demo",
            modelUrn: null,
          },
...
router.get("/history/:projectId", async (req: Request, res: Response) => {
  try {
    // const projectId = req.params.projectId; // TODO: will be used when database retrieval is implemented
