...
export function useWorkflow(
  ...
  const fetchWorkflow = useCallback(async () => {
    ...
    try {
      ...
      const message =
        axiosError.response?.data?.message || "Error al cargar workflow";
  ...
  const performTransition = useCallback(
    async (input: TransitionInput): Promise<boolean> => {
  ...
  const canTransition = useCallback(
    (transitionName: string): boolean => {
      if (!workflow || workflow.status !== "ACTIVE") return false;
      return workflow.availableTransitions.some(
  ...
  const getTransition = useCallback(
    (transitionName: string): AvailableTransition | undefined => {
      return workflow?.availableTransitions.find(
        (t) => t.name === transitionName,
      );
    },
    [workflow],
