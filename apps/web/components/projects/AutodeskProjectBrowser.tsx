...
export function AutodeskProjectBrowser({ onSelect, onCancel }: BrowserProps) {
  ...
  // Data States
  const [hubs, setHubs] = useState<ApsItem[]>([]);
  const [projects, setProjects] = useState<ApsItem[]>([]);

  // Browser State
  const [folders, setFolders] = useState<ApsItem[]>([]); // Current level folders
  const [files, setFiles] = useState<ApsItem[]>([]); // Current level files

  // Selection States
  const [selectedHub, setSelectedHub] = useState<ApsItem | null>(null);
  const [selectedProject, setSelectedProject] = useState<ApsItem | null>(null);
  const [selectedFolder, setSelectedFolder] = useState<ApsItem | null>(null); // The target folder to link
  ...
  const loadTopFolders = async (hubId: string, projectId: string) => {
    ...
    try {
      const res = await apiClient.get(
        `/api/aps/hubs/${hubId}/projects/${projectId}/topFolders`,
      );
      setFolders(res.data);
  ...
  const loadFolderContents = async (projectId: string, folderId: string) => {
    ...
    try {
      const res = await apiClient.get(
        `/api/aps/projects/${projectId}/folders/${folderId}/contents`,
      );

      // Separate folders and files
      const rawContents = res.data;
      const newFolders = rawContents.filter(
        (item: ApsItem) => item.type === "folders",
      );
      const newFiles = rawContents.filter(
        (item: ApsItem) => item.type === "items",
      );
  ...
  const handleHubClick = (hub: ApsItem) => {
    setSelectedHub(hub);
    setPath([{ id: "hub", name: hub.name }]);
    loadProjects(hub.id);
  };

  const handleProjectClick = (project: ApsItem) => {
  ...
  const handleFolderEnter = (folder: ApsItem) => {
    // setCurrentFolderId removed
          ...
          <div className="flex items-center text-sm">
            ...
            {path.map((p, i) => (
              <div key={i} className="flex items-center whitespace-nowrap">
                ...
                <span
                  className={`truncate max-w-[150px] ${i === path.length - 1 ? "font-semibold text-foreground" : "text-muted-foreground"}`}
                >
                  {p.name}
                </span>
              </div>
      ...
      <div className="flex-1 flex gap-4 min-h-0">
                ...
                <>
                      ...
                      {(!error ||
                        ...
                        <div className="mt-4 p-3 bg-muted/50 rounded-lg border border-border/50">
                          <p className="text-xs text-muted-foreground">
                            Tip: Ensure you have access to a Hub. Try using the
                            User Menu (top right) to <b>Switch Account</b>{" "}
                  ...
                  {step === "browser" &&
                    folders.map((folder) => (
                      ...
                      >
                        <Folder
                          className={`w-4 h-4 mr-3 ${selectedFolder?.id === folder.id ? "text-primary fill-primary/20" : "text-muted-foreground"}`}
                        />
        ...
        <div className="w-1/2 flex flex-col">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase mb-2 px-1 flex items-center justify-between">
            ...
            {selectedFolder && (
              <span className="text-primary truncate max-w-[150px] ml-2">
                {selectedFolder.name}
              </span>
            )}
          </h3>
          <ScrollArea className="flex-1 border border-border rounded-xl bg-card">
            <div className="p-2 space-y-1">
              {previewLoading ? (
                ...
                files.map((file: ApsItem) => (
                  <div
                    key={file.id}
                    className="p-2.5 rounded-lg border border-border/50 bg-secondary/20 flex items-center text-sm"
                  >
