...
router.get("/model-status", async (req, res) => {
  try {
    ...
    const { modelDerivativeService } =
      await import("../services/aps/model-derivative.service");
    ...
    console.error("Manifest Check Error:", err.message || "Unknown error");
    ...
    if (err.response?.status === 404) {
      return res.json({
        status: "failed",
        messages: [
          { type: "error", message: "Model Not Found in Autodesk (404)" },
        ],
      });
    }
    if (err.response?.status === 400) {
      return res.json({
        status: "failed",
        messages: [{ type: "error", message: "Invalid URN Format (400)" }],
      });
    }
...
router.post("/analyze/mop", upload.single("file"), async (req, res) => {
  try {
    ...
    // 1. Extract Text
    const docResult = await documentParserService.parseDocument(
      filePath,
      req.file.mimetype,
    );
...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      const normFile = files["normativeFile"]
        ? files["normativeFile"][0]
        : null;
      ...
      if (projectFileId && !specFile) {
        ...
        if (!dbFile.apsUrn || dbFile.apsUrn.startsWith("local-")) {
          return res.status(400).json({
            error: "Project file not available for analysis (no URN)",
          });
        }

        // Decode URN to get object key
        const decodedUrn = Buffer.from(dbFile.apsUrn, "base64").toString(
          "utf-8",
        );
        const match = decodedUrn.match(
        ...
        if (!objectKey) {
          return res
            .status(400)
            .json({ error: "Invalid file record (missing object key)" });
        }
        ...
        if (!signedUrl) {
          return res
            .status(500)
            .json({ error: "Failed to get download URL for project file" });
        }
        ...
        console.log(
          `[Analyze] Fetched project file: ${dbFile.name} (${dbFile.size} bytes)`,
        );
      }
      ...
      if (!specFile && !normFile) {
        return res
          .status(400)
          .json({ error: "No specification or normative file uploaded" });
      }

      // Lazy load services
      const { HierarchicalParserService } =
        await import("../services/hierarchical-parser.service");
      const { HierarchicalSpecProcessor } =
        await import("../services/hierarchical-spec-processor");
      const { NormativeParserService } =
        await import("../services/normative-parser.service");
      const { SupremacyEngineService } =
        await import("../services/supremacy-engine.service");
      ...
      if (specFile) {
        ...
        if (specFile.mimetype === "application/pdf") {
          console.log(
            `[Analyze] Parsing Spec PDF: ${specFile.originalname} (${specFile.size} bytes)`,
          );
          const tree = await hierarchyParser.parse(specFile.path);
          console.log(
            `[Analyze] Tree built with ${tree.length} top-level nodes.`,
          );
          ...
          console.log(
            `[Analyze] Extracted ${specRequirements.length} requirements from PDF.`,
          );
          if (specRequirements.length > 0) {
            console.log(
              `[Analyze] Sample Req:`,
              JSON.stringify(specRequirements[0], null, 2),
            );
          } else {
            console.warn(
              `[Analyze] WARNING: No requirements extracted from PDF!`,
            );
          }
          ...
          const { documentParserService } =
            await import("../services/document-parser.service");
          const { SpecParserService } =
            await import("../services/spec-parser.service");
          const specParser = new SpecParserService();
          const docResult = await documentParserService.parseDocument(
            specFile.path,
            specFile.mimetype,
          ...
          specRequirements = result.requirements.map((r) => ({
            ...r,
            source: "Spec",
          }));
        }
      ...
      if (normFile) {
        console.log(
          `[Analyze] Parsing Normative PDF: ${normFile.originalname}`,
        );
      ...
      if (normRequirements.length > 0) {
        console.log(
          `[Supremacy] Resolving ${specRequirements.length} Spec Rules vs ${normRequirements.length} Norm Rules...`,
        );
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        stats.conflictsResolved = (finalRequirements as any[]).filter(
          (r: { source?: string }) => r.source === "Merged",
        ).length;
      } else {
...
router.post("/verify", upload.single("file"), async (req, res) => {
  try {
    ...
    const safeUrn = toSafeUrn(urn);
    console.log(
      `🚀 Starting Verification. Mode: ${req.body.checklist ? "Interactive (Checklist)" : "Automatic (File)"}`,
    );
      ...
      try {
        requirements =
          typeof req.body.checklist === "string"
            ...
            : req.body.checklist;
        console.log(
          `[Verify] Using cached checklist with ${requirements.length} rules.`,
        );
      } catch {
    ...
    if (discipline !== "ALL") {
      console.log(
        `[Verify] Filtering requirements for discipline: ${discipline}`,
      );
      requirements = requirements.filter(
        (req: { derivedCategory?: string }) => {
    ...
    if (err.message && err.message.includes("APS_MODEL_NOT_READY")) {
      return res.status(409).json({
        error: "Model Processing",
        message: err.message,
        details:
          "The BIM model is still being processed by Autodesk. Please try again in a few minutes.",
      });
    }
    ...
    if (err.response?.status === 400) {
      return res.status(400).json({
        error: "Invalid URN Format (Autodesk Rejected)",
