...
export class ComplianceKernelService {
  ...
  evaluate(
    ...
    for (const req of requirements) {
      ...
      const relevantElements = this.filterElementsByScope(
        elements,
      ...
      for (const elem of relevantElements) {
        ...
        const matchedPropKey = this.findMatchingProperty(
          elem.properties,
          req.parameter,
        );
  ...
  private filterElementsByScope(
    ...
    return elements.filter((elem) =>
      this.isScopeMatch(reqCategory, elem.category, elem.name),
    );
  }
  ...
  private isElectricalCategory(category: string): boolean {
    ...
    return this.ELECTRICAL_CATEGORIES.some((ec) =>
      c.includes(ec.toLowerCase()),
    );
  }
  ...
  private findMatchingProperty(
    ...
    // Fuzzy fallback
    return (
      Object.keys(props).find((k) => k.toLowerCase().includes(search)) || null
    );
  }
   ...
   */
  private checkCompliance(req: UniversalRequirement, actual: unknown): boolean {
  ...
  private determineSeverity(
    req: UniversalRequirement,
    actual: unknown,
  ): "CRITICAL" | "WARNING" {
    // Parse values
    const actualNum = parseFloat(String(actual).replace(",", "."));
    const reqNum = parseFloat(String(req.value).replace(",", "."));
