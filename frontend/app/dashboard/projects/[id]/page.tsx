...
import { ArrowLeft, Upload, File, FileText, Box, Layers, MoreVertical, RefreshCw, Clock, HardDrive, Cloud, CheckCircle, Trash2, List } from "lucide-react"
...
export default function ProjectDetailPage() {
    ...
    const [downloadModal, setDownloadModal] = useState<{ isOpen: boolean, url: string, format: string } | null>(null)
            ...
            const pollInterval = setInterval(async () => {
                try {
                    ...
                    if (status === 'COMPLETED') {
                        ...
                        // Try direct navigation for download (iframe trick to avoid popup blockers)
        ...
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 animate-slide-up stagger-2">
                    <div className="bg-white dark:glass-panel border border-gray-200 dark:border-white/10 rounded-2xl min-h-[500px] flex flex-col border-l-4 border-green-500 shadow-sm">
                        <div className="p-6 border-b dark:border-white/10 border-gray-200 flex justify-between items-center dark:bg-gradient-to-r dark:from-white/5 dark:to-transparent bg-gradient-to-r from-green-50/30 to-transparent">
                            ...
                            <span className="text-xs dark:text-gray-400 text-gray-600 dark:bg-white/10 bg-green-50 px-3 py-1.5 rounded-full border dark:border-white/10 border-green-200 font-medium">{project.files.length} items</span>
                        </div>

                        <div className="p-6 flex-1">
                            {project.files.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full dark:text-gray-500 text-gray-400 py-20">
                                    <div className="bg-gradient-to-br dark:from-white/10 dark:to-white/5 from-gray-100 to-gray-50 p-8 rounded-2xl mb-6 animate-float border-2 dark:border-white/10 border-gray-200">
                                        <Box className="h-16 w-16 dark:text-gray-500 text-gray-400" />
                                    </div>
                                    <p className="text-xl font-medium dark:text-gray-300 text-gray-600">No files uploaded yet</p>
                                    <p className="text-sm dark:text-gray-500 text-gray-500 mt-2">Upload a file or import from Autodesk</p>
                                </div>
                            ) : (
                                    ...
                                    {project.files.map((file, index) => (
                                        ...
                                        >
                                            <div className="flex items-center space-x-4">
                                                ...
                                                <div>
                                                    <p className="font-bold dark:text-white text-gray-900 text-base group-hover:text-dom-blue dark:group-hover:text-dom-blue-light transition-colors">{file.name}</p>
