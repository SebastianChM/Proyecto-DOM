...
export function UserMenu() {
    ...
    const handleLogout = async () => {
        try {
            ...
            if (user) {
                localStorage.setItem('dom_last_user', JSON.stringify({
                    name: user.name,
                    email: user.email,
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 glass-panel border-white/10">
                ...
                <DropdownMenuItem onClick={() => router.push('/dashboard/profile')} className="cursor-pointer focus:bg-primary/10 focus:text-primary">
