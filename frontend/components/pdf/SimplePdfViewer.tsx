...
export function SimplePdfViewer({ file, fileUrl: propFileUrl, highlight }: SimplePdfViewerProps) {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            const targetText = highlight.text.toLowerCase().replace(/\s+/g, ' ').trim();

            let found = false;
            spans.forEach((span) => {
                const spanText = (span.textContent || '').toLowerCase().replace(/\s+/g, ' ').trim();
                const isMatch = spanText.length > 3 && (targetText.includes(spanText) || spanText.includes(targetText));
            ...
            if (!found) console.warn("[SimplePdfViewer] No match found for:", targetText);
        }, 800);
    ...
    if (!fileUrl) {
        return <div className="flex items-center justify-center h-full text-muted-foreground">No PDF loaded</div>;
    }
